...
export const AdminDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-display font-bold text-text-dark mb-1">Tableau de bord</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat, i) => (
          <motion.div 
            ...
            className="bg-white p-4 rounded-2xl border border-border shadow-sm hover:shadow-md transition-shadow"
          >
            <div className="flex items-start justify-between">
              <div>
                <p className="text-text-muted text-[10px] font-bold uppercase tracking-wider mb-1">{stat.label}</p>
                <h3 className="text-2xl font-display font-bold text-text-dark">{stat.value}</h3>
              </div>
              <div className={`p-2.5 rounded-xl ${stat.bg} ${stat.color}`}>
                <stat.icon size={18} />
              </div>
            </div>
            <div className="mt-4 flex items-center gap-1.5 text-[9px] font-bold text-blue-600 bg-blue-50 w-fit px-1.5 py-0.5 rounded-md">
              <TrendingUp size={10} />
              +2 cette semaine
            </div>
          </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-border overflow-hidden shadow-sm flex flex-col h-full min-h-[350px]">
            <div className="flex-grow divide-y divide-border">
              {notifications.map((notif) => (
                <div key={notif.id} className="p-4 flex items-start gap-3 hover:bg-bg-light transition-colors">
                  ...
                  <div className="flex-grow">
                    <p className="text-xs font-semibold text-text-dark mb-0.5">{notif.text}</p>
